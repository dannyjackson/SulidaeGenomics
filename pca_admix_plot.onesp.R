...
p_species <- ggplot(scores, aes(x = PC1, y = PC2, fill = Species)) +
  geom_point(shape = 21, size = 20, alpha = 0.9, color = "white", stroke = 0.3) +
  scale_fill_manual(values = species_cols, drop = FALSE) +
  labs(
    x = paste0("PC1 (", PC1.PV, "%)"),
    y = paste0("PC2 (", PC2.PV, "%)")
  ) +
  geom_text(aes(label = LabelNum), size = 16, color = "white")+
...
q_candidates <- Sys.glob(file.path(grp_dir, sprintf("%s.k%s.admix.%s.Q", GROUP, KVAL, KVAL)))
...
out_admix <- file.path(grp_dir, sprintf("%s.k%s.admix.pdf", GROUP, KVAL))
pdf(file = out_admix, width = 4, height = 4, family = "Helvetica")
