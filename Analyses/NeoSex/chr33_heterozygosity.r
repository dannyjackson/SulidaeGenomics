...
# compare heterozygosity across chr33
cd /xdisk/mcnew/dannyjackson/sulidae/datafiles/bamstats/sexing_by_heterozygosity_filtered
mkdir -p hetplot_chr33
cd hetplot_chr33

# inputs
VCF=/xdisk/mcnew/dannyjackson/sulidae/analyses/raxml_wg/vcfs/chroms/"${CHR}".qualitysort_filtered_mind2.vcf        # bgzipped + indexed VCF
# VCF=/xdisk/mcnew/dannyjackson/sulidae/analyses/raxml_wg/vcfs/Sula_MorusBassanus_snps_multiallelic.contigs.qualitysort.vcf
CHR=CM062599.1       # chromosome/scaffold name
WIN=10000            # window size
STEP=10000           # step size (non-overlapping)
...
win <- read_tsv("CM062599.1.pi_10000.windowed.pi",
...
ggsave("pi.CM062599.1.allindv.png", width = 12, height = 24, dpi = 300)


### Per individual heterozygosity
# --- inputs you edit ---
CHR=CM062599.1                    # chromosome/scaffold to plot (change as needed)
...
WIN <- 50000
OUT <- "hets_CM062599.1"   # must match the bash OUT above (or set dynamically)
...
p <- ggplot(hets_sex,
  ...
  labs(
    title = paste0("Chromosome 33 (50 kb bins)"),
...
ggsave(paste0(OUT, ".hets_50kb.pdf"), p, width = 6, height = 3, dpi = 300)
