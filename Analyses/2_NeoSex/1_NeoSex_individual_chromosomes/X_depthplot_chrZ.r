cd /xdisk/mcnew/dannyjackson/sulidae/datafiles/bamstats/sexing_by_depth_filtered/depthplot_chrZ

while read -r ind; do
echo ${ind}
grep 'CM062600.1' /xdisk/mcnew/dannyjackson/sulidae/datafiles/bamstats/${ind}_depthstats.txt > ${ind}_CM062600.1_depth.txt
done < /xdisk/mcnew/dannyjackson/sulidae/referencelists/allsamplecodes.txt


CHR="CM062600.1"   # chromosome to analyze
WIN=1000000          # window size in bp (10k–100k typical)
...
bins_file <- "depth_summaries/binned_depth.CM062600.1.WIN1000000.tsv"
samp_file <- "depth_samples/sampled_depth.CM062600.1.p.tsv"  # optional
...
p1 <- ggplot(bins, aes(x = bin_mid, y = depth_norm, group = individual)) +
  ...
  labs(x = "Position on CM062600.1 (bp)",
...
print(p1)
ggsave("depth_profiles.CM062600.1.binned_norm.png", p1, width = 12, height = 8, dpi = 300)
...
p1 <- ggplot(bins, aes(x = bin_mid, y = depth_norm, color = species, group = individual)) +
  ...
  labs(
    x = "Position on CM062600.1 (bp)",
    y = "Normalized mean depth (per individual median = 1)",
    title = "Depth profiles across CM062600.1 by species",
    color = "Species"
  ) +
...
print(p1)
ggsave("depth_profiles.CM062600.1.all_species.png", p1, width = 12, height = 6, dpi = 300)
...
ggsave("depth_profiles.CM062600.1.faceted_species_indiv_legend_inpanel.png",
...
ggsave("depth_profiles.CM062600.1.faceted_species_indiv_legend_inpanel.trim98.png",
       p_all, width = 24, height = 12, dpi = 300)
