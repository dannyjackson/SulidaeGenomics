...
WIN <- 50000
OUT <- "hets_wg"   # must match the bash OUT above (or set dynamically)

samples <- readLines(paste0(OUT, ".samples.txt"))
colnames_geno <- c("CHROM","POS", samples)

print("Reading genotype table")
...
print("Converting genotype table to long format")
# Long format
long <- geno |>
...
print("Defining heterozygote flag")
...
print("Binning positions to 50kb")
...
write.csv(hets, "50kb_heterozygosity.csv")
...
print("Filter out low-data windows")
...
chr_file  <- "/xdisk/mcnew/dannyjackson/sulidae/referencelists/chromconversion.txt"

sex <- read_csv(sex_file, col_names = c("individual", "sex"))

print("Joining with sex and chr")
...
write.csv(hets, "50kb_heterozygosity.labelled.csv")